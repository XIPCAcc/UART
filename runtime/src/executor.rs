...
impl Task {
    ...
    fn wake_by_ref_(self: &Rc<Self>) {
        ...
        if prev & STATE_SPAWNED == 0 {
            // 任务已完成（poll 返回 Ready 时 SPAWNED 被清除）：丢弃这次唤醒，
            // 不再入队，避免对已完成的 future 再次 poll 触发 panic。
            return;
        }
    ...
    fn drain_queue(&self) {
        loop {
            ...
            while !node.is_null() {
                ...
                // 任务在最后一次 poll 中已完成（SPAWNED 被清除），但完成前的
                // 竞态唤醒仍可能把它残留入队。直接释放该任务，不再 poll，
                // 避免对已完成的 future 再次 poll 触发「async fn resumed
                // after completion」panic。
                if unsafe { (*node).state.load(Ordering::Acquire) } & STATE_SPAWNED == 0 {
                    drop(unsafe { Rc::from_raw(node as *const Task) });
                    node = next;
                    continue;
                }
                ...
                if is_ready {
                    // 标记任务完成：清除 SPAWNED。此后任何残留 waker 的唤醒
                    // 都会被 wake_by_ref_ 丢弃，不会把已完成的 future 重新入队。
                    task.raw.state.fetch_and(!STATE_SPAWNED, Ordering::Release);
                    self.pending.set(self.pending.get() - 1);
                }
